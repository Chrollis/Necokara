...
1. **先说行不行，再说做不做** — 对于每一条指令，先分析可行性给出回答，再等待进一步指令。
2. **先说明方案再动手** — 重构或新增功能前先提方案，确认后再执行。
...
- **样式**: 使用普通 CSS，不引入 Sass/Less、CSS Modules 等预处理方案。颜色变量统一放在 `:root`。
...
- `Word.reading` 和 `Syllable.reading` 中禁止空白字符
- 空格/换行只能通过静态工厂方法生成
- 歌词末尾必须有换行符
- 分隔符不得连续
