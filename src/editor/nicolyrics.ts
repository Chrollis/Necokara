...
import {
  ...
  createSpaceWord,
  createNewlineWord,
  createPunctuationWord,
} from './word';
import { isPunctuationChar } from './syllable';
import { createTime, parseTime, formatTime } from './time';

/** Split a timestamped chunk into ordinary-text / punctuation segments. */
  ...
  try {
    ...
    for (let lineIdx = 0; lineIdx < lines.length; lineIdx += 1) {
      ...
      if (line.includes('[')) {
        ...
        // Leading text before the first timestamp (e.g. `「` at line start)
        // must not drop the whole line — keep it as a leading word.
        const firstBracket = line.indexOf('[');
        const prefix = firstBracket > 0 ? line.slice(0, firstBracket) : '';
        ...
        // keep the leading prefix (`「`) as the first word of the line
        const cleanedPrefix = prefix.replace(/[\s\u3000]/g, '');
        if (cleanedPrefix.length > 0) {
          lineWords.unshift({ time: firstTime, text: cleanedPrefix });
        }
    ...
    pendingLines.forEach((line) => {
      line.words.forEach((entry) => {
        ...
        // split trailing / embedded punctuation (`ね」`) into separate words
        const parts = splitPunctuation(wordText);
        for (const part of parts) {
          if (isPunctuationChar(part)) {
            words.push(createPunctuationWord(part, createTime(ms)));
            continue;
          }

          const entries = rubyMap.get(part) || [];
          ...
          if (matched) {
            ...
            word = createWord(part, createTime(ms));
